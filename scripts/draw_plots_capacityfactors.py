...
def sort_timeseries(data, ascending=True):
    ...
    for column in data.columns:
        sorted_data[column] = (
            sorted_data[column].sort_values(ascending=ascending).values
        )
...
def plot_annual_average(capacity_factors, labels=None, fig=None, ax=None):
    ...
    df = (
        capacity_factors.groupby("time.year")
        .mean("time")
        .to_dataframe()["__xarray_dataarray_variable__"]
        .unstack("year")
    )
    ...
    for n, col in enumerate(df.columns):
        ax.plot(
            df[col], label=col, linestyle="none", marker="o", alpha=0.5, color=cmap(n)
        )
...
if __name__ == "__main__":
    ...
    if snakemake.wildcards.tech in [
        "offshore_deep_awe",
        "offshore_shallow_awe",
        "offshore_deep_hawt",
    ]:
        labels = boundaries.set_index("id")[["iso_sov1", "iso_sov2"]].apply(
            format_tuple, axis=1
        )
        capacity_factors = relabel_index(capacity_factors, "id", labels)
        labels = None

    elif snakemake.wildcards.tech in [
        "onshore_awe",
        "onshore_hawt",
        "old_wind-onshore",
        "old_wind-offshore",
        ...
        capacity_factors = (
            relabel_index(capacity_factors, "id", relabel_dict).groupby("id").mean()
        )
        labels = None
    ...
    df_capacity_factors = capacity_factors.to_dataframe()[
        "__xarray_dataarray_variable__"
    ].unstack("id")
    df_capacity_factors["year"] = df_capacity_factors.index.year
    df_capacity_factors["month-day-hour"] = df_capacity_factors.index.strftime(
        "%m %d %H"
    )
    ...
    df = pd.pivot_table(
        df, index=df["month-day-hour"], columns="year", values=df.columns
    )
